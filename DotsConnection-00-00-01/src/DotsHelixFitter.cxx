...
// DotsHelixFitter::DotsHelixFitter() : myHelix(NULL), myIsInitialized(false), myMdcGeomSvc(NULL), myCgemGeomSvc(NULL), myMdcCalibFunSvc(NULL), myMdcUtilitySvc(NULL)
DotsHelixFitter::DotsHelixFitter() : myHelix(NULL), myIsInitialized(false), myMdcGeomSvc(NULL), myMdcCalibFunSvc(NULL), myMdcUtilitySvc(NULL)
{
...
void DotsHelixFitter::initialize()
{
    ...
    // if (Cgem_sc.isSuccess())
    //     myCgemGeomSvc = dynamic_cast<CgemGeomSvc *>(ISvc);
    // else
    //     cout << "DotsHelixFitter::initialize(): can not get CgemGeomSvc" << endl;
    ...
    if (mdc_sc.isSuccess())
        myMdcGeomSvc = dynamic_cast<MdcGeomSvc *>(ISvc2);
    else
        cout << "DotsHelixFitter::initialize(): can not get MdcGeomSvc" << endl;
    int nLayer = myMdcGeomSvc->getLayerSize();
    if (nLayer != 43)
        cout << "DotsHelixFitter::initialize(): MDC nLayer = " << nLayer << " !=43  !!!" << endl;
...
int DotsHelixFitter::calculateNewHelix()
{
    ...
    for (int i = 0; i < 43; i++)
    {
        if (maxRTrk < myRWires[i] + 2)
            break; // where soft track turning back
    ...
    if (myFitCircleOnly)
    {
        if (nXHits[0] < myMinXhitsInCircleFit)
            return 1;
    }
    else
    {
        if ((nXHits[0] + nVHits[0]) < myMinHitsInHelixFit)
            return 2;
        if (nVHits[0] < myMinVhitsInHelixFit)
            return 3;
    }
    ...
    while (1) // iterations
    {
        ...
        for (; iter_mdcDigi != myVecMdcDigi.end(); iter_mdcDigi++, i_digi++)
        {
            ...
            double wpos[7] = {myEastPosWires[layer][wire][0], myEastPosWires[layer][wire][1], myEastPosWires[layer][wire][2], myWestPosWires[layer][wire][0], myWestPosWires[layer][wire][1], myWestPosWires[layer][wire][2], tension};
            double doca_trk;
            ...
            while (entranceAngle < -M_PI)
                entranceAngle += 2 * M_PI;
            while (entranceAngle > M_PI)
                entranceAngle -= 2 * M_PI;
            ...
            if (!myMdcDigiIsActive[i_digi])
                continue;
            if (myUseAxialHitsOnly && myWireFlag[layer] != 0)
                continue;
        ...
        if (debug)
            cout << "end of MDC hits loop" << endl;
        ...
        //     while (incidentPhi > CLHEP::pi)
        //         incidentPhi -= CLHEP::twopi;
        //     while (incidentPhi < -CLHEP::pi)
        //         incidentPhi += CLHEP::twopi;
        ...
        //         if (debug)
        //             cout << "phi_trk, phi_rec = " << phi_trk << ", " << phi_CC << endl;
        ...
        //         if (debug)
        //             cout << "V_trk, V_rec = " << V_trk << ", " << V_CC << endl;
        ...
        //             if (fabs(delta_m_2) < fabs(delta_m))
        //                 delta_m = delta_m_2;
        //             if (debug)
        //                 cout << "V_trk_nearPhiMin= " << V_trk_nearPhiMin << endl;
        //         }
        //         err_m = myCgemCalibSvc->getSigma(layer, flag, mode, incidentPhi, Q, T) / 10.; // in cm
        ...
        {
            if (debug)
                cout << "DotsHelixFitter::calculateNewHelix(): " << nIterations
...
void DotsHelixFitter::updateDcDigiInfo(const MdcDigi *aDcDigi)
{
    ...
    if (myDocaFromTrk != 0)
    {
        ...
        if (myLeftRight == 1)
            myLeftRight = 0; // fixed 2020-11-26
        else
            myLeftRight = 1;
    ...
    if (debug)
        cout << "myLeftRight = " << myLeftRight << endl;
    ...
    if (debug)
        cout << "doca_hit = " << myDocaFromDigi << endl;
...
vector<RecMdcHit> DotsHelixFitter::makeRecMdcHitVec(int sel)
{
    ...
    for (; iter_mdcDigi != myVecMdcDigi.end(); iter_mdcDigi++, i_digi++)
    {
        if (sel == 1 && !myMdcDigiIsActive[i_digi])
            continue; // skip inactive hits
